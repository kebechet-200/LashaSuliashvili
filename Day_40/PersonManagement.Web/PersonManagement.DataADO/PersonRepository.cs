    ...
    public class PersonRepository : IPersonRepository
    {
        ...
        public async Task<List<Person>> GetAllAsync()
        {
            ...
            using (SqlConnection connection = new SqlConnection(_connection))
            {
                ...
                SqlDataReader reader = await command.ExecuteReaderAsync(); //query command იგზავნება ბაზაsთან და გენერირდება ჩანაწერები

                while (await reader.ReadAsync()) // კითხულობs ჩანაწერებs აsინქრონულად
                {
